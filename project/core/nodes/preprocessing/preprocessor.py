...
class Preprocessor:
    ...
    def create_preprocessor(self):
        ...
        preprocessor = preprocessors[self.preprocessor_type][self.preprocessor_name]["node"](**self.params)
        payload = { "message":f"{self.preprocessor_type} created {self.preprocessor_name}", "params": self.params,
                   "node_name": self.preprocessor_name, "node_type": self.preprocessor_type, 
                   "node_id": id(preprocessor), "node": preprocessor
                   }
        save_node(payload)
        del payload['node']
        return payload
